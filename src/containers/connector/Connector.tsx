...
const useConnector = () => {
  ...
  const updateState = useCallback((update: AppState) => {
    if (update.wallets.length > 0) {
      ...
      const network: Network = {
        ...
        label: networkId === 1 ? 'Ethereum' : 'Optimism',
      };

      const provider = new ethers.providers.Web3Provider(
        update.wallets[0].provider,
        {
          name: network.name,
          chainId: networkId,
        },
  ...
  useEffect(() => {
    const previousWalletsSerialised = localStorage.getItem(
      LOCAL_STORAGE_KEYS.SELECTED_WALLET,
    );
    ...
    if (onboard) {
      ...
      return () => {
        if (process.env.NODE_ENV !== `development` && unsubscribe)
          unsubscribe();
      };
    }
  ...
  useEffect(() => {
    if (walletAddress && !ensName) {
      (async () => {
        const ensN: string | null = await L1DefaultProvider.lookupAddress(
          walletAddress,
        );
        const ensA = ensName
          ? await L1DefaultProvider.getAvatar(ensName)
          : null;
        if (ensN) {
          dispatch({
            type: AppEvents.SET_ENS,
            payload: { ensName: ensN, ensAvatar: ensA },
          });
        }
      })();
    }
  ...
  const setWatchedWallet = useCallback(
    (
      ...
      dispatch({
        type: AppEvents.WATCH_WALLET,
        payload: { address, walletWatched, ensName },
      });
    },
    [],
  );
