...
function Container() {
  ...
  useEffect(() => {
    ...
    const loadLoans = async () => {
      ...
      const openLoans = await getOpenLoans({
        ...
        isL2,
        subgraphOpenLoans: subgraphOpenLoansKey
          ? JSON.parse(subgraphOpenLoansKey)
          : [],
      });
      if (isMounted) {
        setLoans(openLoans);
    ...
    const subscribe = () => {
      ...
      const collateralDepositedEvent =
        loanContract.filters.CollateralDeposited(walletAddress);
      const collateralWithdrawnEvent =
        loanContract.filters.CollateralWithdrawn(walletAddress);
      const loanDrawnDownEvent =
        loanContract.filters.LoanDrawnDown(walletAddress);
      const loanRepaymentMadeEvent =
        loanContract.filters.LoanRepaymentMade(walletAddress);
  ...
  const rateAndDelayQueries = useQuery(
    ...
    async () => {
      ...
      const [[borrowRate], ethBorrowIssueFeeRate, ethInteractionDelay] =
        await Promise.all([
